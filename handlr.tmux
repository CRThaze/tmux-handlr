...
set_binds() {
	...
	# Restart the detection daemon; opt-in, empty = no bind.
	restart_key="$(get_tmux_option '@handlr-restart-key' '')"
	...
	if [ -n "$restart_key" ]
	then
		tmux bind-key "$restart_key" run-shell "$CURRENT_DIR/scripts/agent-daemon-restart.sh"
	fi
}
